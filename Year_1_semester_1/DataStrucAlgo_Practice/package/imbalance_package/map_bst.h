...
class map_bst
{
  ...
  public:
    ...
    int process(node *root, KeyT& ans, int& max_imbalance) {
    // Your code here
    if(root==NULL) return -1;

    int hlef = process(root->left,ans,max_imbalance);
    int hrig = process(root->right,ans,max_imbalance);
    int imb = hlef-hrig;
    if(imb < 0 ) imb = -imb;
    int h = 1+((hlef>hrig)? hlef:hrig);
    ...
    KeyT getValueOfMostImbalanceNode() {
        ...
        process(mRoot,ans,max_imbalance);
        return ans;
    }
